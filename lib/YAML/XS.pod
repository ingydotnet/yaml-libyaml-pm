...
It implements the YAML 1.2 Core Schema, while the old interface
...
The functional YAML::XS interface only recognizes the following special values:
...
=head3 dump_string
    ...
    $yaml = $xs->dump_string($data);
    $yaml = $xs->dump_string($data1, $data2, $data3);
    $yaml = $xs->dump_string(@data);
